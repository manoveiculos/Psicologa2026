...
export async function atualizarDadosPaciente(patientId: string, data: any) {
  ...
  const { error } = await sb
    ...
    .update({
      ...
      tipo_atendimento: data.tipo_atendimento || 'particular',
      convenio: data.convenio || data.convenio_nome || null,
      convenio_nome: data.convenio_nome || data.convenio || null,
      valor_sessao_default: data.valor_sessao_default ? Number(data.valor_sessao_default) : null,
      valor_convenio: data.valor_convenio !== "" && data.valor_convenio != null ? Number(data.valor_convenio) : null,
      duracao_convenio_min: data.duracao_convenio_min ? Number(data.duracao_convenio_min) : null,
      valor_particular: data.valor_particular !== "" && data.valor_particular != null ? Number(data.valor_particular) : null,
      duracao_particular_min: data.duracao_particular_min ? Number(data.duracao_particular_min) : 50,
      cpf: data.cpf || null,
